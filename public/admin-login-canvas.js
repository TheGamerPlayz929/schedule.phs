/* One dot/letter field: glyphs morph in place without a dark covering canvas. */
(() => {
  'use strict';
...
void main() {
  ...
  if (u_use_cell_state && !u_cell_pass) {
    ...
    if (reveal > 0.001) {
      ...
      /* Preserve the field's light in the emerging strokes instead of darkening a circle. */
      opacity = mix(opacity, max(opacity, 0.38), morph);
    }
  ...
  function draw(now) {
    ...
    lastNow = now;
    strength = motion.matches ? 0 : Math.max(0, Math.min(1, strength + (active ? dt / 60 : -dt / 150)));
    if (dirty) {
      revision += 1;
      dirty = false;
    }
    ...
    gl.uniform1f(hoverLocation, strength);
